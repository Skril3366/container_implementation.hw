...
COMMON_LABELS = (
    GENERAL_STATS_LABELS + LATENCY_STATS_LABELS + THREADS_FAIRNESS_LABELS
)
FILEIO_LABELS = [
    'ops reads/s',
    'ops writes/s',
...
def benchmark(
    func: ParseFunc, cmd: str, cols: list[str], n: int = 10
) -> Table:
    table = Table(cols)
...
if __name__ == '__main__':
    ...
    cmd_cpu = 'sysbench cpu --threads=10 --time=20 --cpu-max-prime=64000 run'
    table_cpu = benchmark(
        test_primes, cmd_cpu, ['CPU events/s'] + COMMON_LABELS
    )
    ...
    cmd_fileio = 'sysbench fileio --file-total-size=1G --file-test-mode=rndrw --time=30 --time=30 --max-requests=0 run'

    table_fileio = benchmark(
        test_fileio, cmd_fileio, FILEIO_LABELS + COMMON_LABELS
    )
